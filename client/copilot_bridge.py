"""HiveShip Copilot Bridge — file-based bridge for Copilot-driven testing.

Usage (from repo root):
    python client/copilot_bridge.py "your goal text here"
    python client/copilot_bridge.py                       # uses default goal
...
The generation pipeline includes HiveShip's learning loop: memory prefetch,
skill discovery, history search (injected into planner), and post-PR extraction
of memory entries and skills. These run automatically — Copilot just needs to
answer the LLM prompts (which will include memory/skill context when available).
...
DEFAULT_GOAL = os.environ.get("DEV_TEST_GOAL", (
...
def main():
    ...
    if not env.get("GITHUB_TOKEN"):
        print(f"{RED}  ERROR: GITHUB_TOKEN env var not set. Set it in .env before running.{RESET}")
        sys.exit(1)
    if not REPO_OWNER or not REPO_NAME:
        print(f"{RED}  ERROR: REPO_OWNER and REPO_NAME must be set in .env{RESET}")
        sys.exit(1)
