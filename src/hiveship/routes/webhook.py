...
from hiveship.config import (
    ...
    DEFAULT_LLM_PROVIDER, DEFAULT_REPO_NAME, DEFAULT_REPO_OWNER,
    ENABLE_TEST_ENDPOINTS, MAX_READ_FILES,
    WEBHOOK_SECRET,
)
from hiveship.git import github_api_request, run_git
from hiveship.engine.job_store import create_job, update_job
from hiveship.llm import create_models, sync_generate_and_parse
from hiveship.models import DeliveryPlan, FileRequest, ReviewResult
...
async def github_webhook(
    ...
    # Extract repo owner/name from the webhook payload itself
    repo_data    = payload.get("repository", {})
    repo_owner   = repo_data.get("owner", {}).get("login", DEFAULT_REPO_OWNER)
    repo_name    = repo_data.get("name", DEFAULT_REPO_NAME)
...
async def test_webhook(
    ...
    repo_owner = (body.repo_owner or "").strip()[:80] or DEFAULT_REPO_OWNER
    repo_name  = (body.repo_name or "").strip()[:80] or DEFAULT_REPO_NAME
...
async def _dispatch_pr_revision(
    ...
    await asyncio.to_thread(
        ...
        job_id, parent_job_id, repo_owner, repo_name,
    )
